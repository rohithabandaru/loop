...
export async function GET() {
  ...
  try {
    ...
    const formatted = themes.map((t) => {
      const allFeedback = t.feedbackThemes.map((ft) => ft.feedback);
      ...
      const avgScore =
        totalCount > 0
          ? Number(
              (
                allFeedback.reduce((acc, curr) => acc + curr.sentimentScore, 0) / totalCount
              ).toFixed(2)
            )
          : 0.0;
    ...
    formatted.sort((a, b) => b.totalCount - a.totalCount);
